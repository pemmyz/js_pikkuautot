...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createTransformedImage(originalImage, scale, rotationDegrees) {
        if (!originalImage || !originalImage.width || !originalImage.height) {
             ...
             placeholderCanvas.width = 10; placeholderCanvas.height = 10;
             return { canvas: placeholderCanvas, width: 10, height: 10 };
        }
        const radians = rotationDegrees * Math.PI / 180;
        const newWidth = originalImage.width * scale; const newHeight = originalImage.height * scale;
        const offscreenCanvas = document.createElement('canvas');
        const offscreenCtx = offscreenCanvas.getContext('2d');
        const absCos = Math.abs(Math.cos(radians)); const absSin = Math.abs(Math.sin(radians));
        ...
        offscreenCtx.rotate(radians); offscreenCtx.scale(scale, scale);
        offscreenCtx.drawImage(originalImage, -originalImage.width / 2, -originalImage.height / 2);
        return { canvas: offscreenCanvas, width: offscreenCanvas.width, height: offscreenCanvas.height };
    }
    ...
    class Car {
        constructor(transformedImageCanvas, screenWidth, screenHeight) {
            if (!transformedImageCanvas || !transformedImageCanvas.canvas) {
                console.error("Car constructor invalid image data:", transformedImageCanvas);
                const ph = document.createElement('canvas'); ph.width = 30; ph.height = 15;
                this.transformedImage = { canvas: ph, width: 30, height: 15 };
            } else { this.transformedImage = transformedImageCanvas; }
            this.width = this.transformedImage.width; this.height = this.transformedImage.height;
            this.x = 0; this.y = 0;
        ...
        getRect() {
            if (typeof this.x !== 'number' || typeof this.y !== 'number' || typeof this.width !== 'number' || typeof this.height !== 'number') {
                console.warn("Car getRect invalid props:", this); return { x: 0, y: 0, width: 1, height: 1 };
            } return { x: this.x, y: this.y, width: this.width, height: this.height };
        }
        move(dx, dy) { this.x += dx; this.y += dy; }
        clampToScreen() {
            if (this.x < 0) this.x = 0; if (this.x + this.width > this.screenWidth) this.x = this.screenWidth - this.width;
            if (this.y < 0) this.y = 0; if (this.y + this.height > this.screenHeight) this.y = this.screenHeight - this.height;
        }
        draw(ctx) {
            if (this.transformedImage && this.transformedImage.canvas && this.transformedImage.canvas.width > 0) {
                ctx.drawImage(this.transformedImage.canvas, this.x, this.y);
                 ...
                 console.warn("Attempt draw car invalid image", this); ctx.fillStyle = 'magenta'; ctx.fillRect(this.x, this.y, 30, 15);
            }
        }
    }
    ...
    class OtherCar extends Car {
        constructor(transformedImageCanvas, screenWidth, screenHeight, playerCar1, playerCar2) {
             ...
             this.x = screenWidth + getRandomInt(50, 400); this.y = getRandomInt(0, screenHeight - this.height);
             this.playerCar1 = playerCar1; this.playerCar2 = playerCar2;
    ...
    class Bullet {
        constructor(x, y, speed) { this.x = x; this.y = y; this.speed = speed; this.width = 10; this.height = 5; this.color = 'red'; this.id = Math.random(); }
     ...
     class RoadMarking {
        constructor(initialX, positionY, screenWidth) { this.screenWidth = screenWidth; this.width = 80; this.height = 10; this.color = 'white'; this.x = initialX; this.y = positionY; }
        moveLeft(speed) { this.x -= speed; if (this.x + this.width < 0) { this.x += this.screenWidth * 1.5 + Math.random() * 200; } }
        draw(ctx) { ctx.fillStyle = this.color; ctx.fillRect(this.x, this.y, this.width, this.height); }
    }
    ...
    class Game {
        ...
        setupRoadMarkings() {
             const numLanes = 5; const laneHeight = this.screenHeight / numLanes; const markingSpacing = 250; const markingsPerScreenRoughly = Math.ceil(this.screenWidth / markingSpacing) + 4;
             for (let i = 0; i < numLanes; i++) { const laneY = (i * laneHeight) + (laneHeight / 2) - 5;
                 for (let j = 0; j < markingsPerScreenRoughly; j++) { const initialX = (j * markingSpacing) - (markingSpacing * 2); this.roadMarkings.push(new RoadMarking(initialX, laneY, this.screenWidth)); }
             }
        }
        ...
        loadNewCars() {
             const now = performance.now(); if (now - this.lastOtherCarLoadTime < this.otherCarLoadInterval) return; this.lastOtherCarLoadTime = now; const currentCarCount = this.otherCars.length; if (currentCarCount >= MAX_CARS) return;
             if (this.availableEnemyImageData.length === 0) { if (this.enemyImagesData.length > 0) { this.availableEnemyImageData = [...this.enemyImagesData]; } else { return; } } if (this.availableEnemyImageData.length === 0) return;
             const availableSlots = MAX_CARS - currentCarCount; const numToAdd = Math.min(availableSlots, 2, this.availableEnemyImageData.length);
             for (let i = 0; i < numToAdd; i++) {
                 if (this.availableEnemyImageData.length === 0) { if (this.enemyImagesData.length > 0) { this.availableEnemyImageData = [...this.enemyImagesData]; } else { break; } } if (this.availableEnemyImageData.length === 0) break;
                 const randomIndex = Math.floor(Math.random() * this.availableEnemyImageData.length); const selectedImageData = this.availableEnemyImageData.splice(randomIndex, 1)[0];
                 if (selectedImageData) { const newCar = new OtherCar( selectedImageData, this.screenWidth, this.screenHeight, this.playerCar1, this.playerCar2 ); let spawnCollision = false; const newCarRect = newCar.getRect(); const spawnBuffer = 10;
                     if (checkCollision(newCarRect, this.playerCar1.getRect()) || checkCollision(newCarRect, this.playerCar2.getRect())) { spawnCollision = true; } else { for (const other of this.otherCars) { const otherRect = other.getRect(); const bufferedOtherRect = { x: otherRect.x - spawnBuffer, y: otherRect.y - spawnBuffer, width: otherRect.width + spawnBuffer * 2, height: otherRect.height + spawnBuffer * 2 }; if (checkCollision(newCarRect, bufferedOtherRect)) { spawnCollision = true; break; } } }
                     if (!spawnCollision) { this.otherCars.push(newCar); } else { this.availableEnemyImageData.push(selectedImageData); }
                 }
             }
        }
        handlePlayerInput() {
            let moveX1 = 0, moveY1 = 0; if (keysPressed['arrowleft']) moveX1 -= this.carSpeed; if (keysPressed['arrowright']) moveX1 += this.carSpeed; if (keysPressed['arrowup']) moveY1 -= this.carSpeed; if (keysPressed['arrowdown']) moveY1 += this.carSpeed; const now = performance.now();
            if (keysPressed[' '] && this.bulletsCar1.length < this.maxBullets) { if (now - this.lastShotTimeCar1 > this.shotCooldown) { const bullet = new Bullet(this.playerCar1.x + this.playerCar1.width, this.playerCar1.y + this.playerCar1.height / 2 - 2.5, 20); this.bulletsCar1.push(bullet); this.lastShotTimeCar1 = now; } }
            ...
            this.playerCar1.move(moveX1, moveY1); this.playerCar1.clampToScreen();
            let moveX2 = 0, moveY2 = 0; if (keysPressed['a']) moveX2 -= this.carSpeed; if (keysPressed['d']) moveX2 += this.carSpeed; if (keysPressed['w']) moveY2 -= this.carSpeed; if (keysPressed['s']) moveY2 += this.carSpeed;
            if (keysPressed['f'] && this.bulletsCar2.length < this.maxBullets) { if (now - this.lastShotTimeCar2 > this.shotCooldown) { const bullet = new Bullet(this.playerCar2.x + this.playerCar2.width, this.playerCar2.y + this.playerCar2.height / 2 - 2.5, 20); this.bulletsCar2.push(bullet); this.lastShotTimeCar2 = now; } }
        ...
        gameLoop(timestamp) { this.update(); this.draw(); requestAnimationFrame(this.gameLoop.bind(this)); }
